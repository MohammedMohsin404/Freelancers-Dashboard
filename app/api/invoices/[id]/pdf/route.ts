...
// Ensure we convert any Uint8Array<ArrayBufferLike> into a plain ArrayBuffer
...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Header
    page.drawRectangle({ x: 0, y: 790, width, height: 30, color: rgb(0.31, 0.34, 0.90) });
    page.drawText("INVOICE", { x: 40, y: 798, size: 16, font: fontBold, color: rgb(1, 1, 1) });
    ...
    page.drawText(inv.status, {
      x: boxX + 12,
      y: boxY + 12,
      size: 12,
      font: fontBold,
      color: inv.status === "Paid" ? rgb(0.2, 0.6, 0.2) : rgb(0.85, 0.6, 0.15),
    });
    ...
    // Save & return: copy to a plain ArrayBuffer to avoid SharedArrayBuffer typing issues
    const pdfBytes = await pdf.save();           // Uint8Array<ArrayBufferLike>
    const ab = toArrayBuffer(pdfBytes);          // ArrayBuffer

    // You can return either Response(ArrayBuffer) or Blob; both are BodyInit-safe
